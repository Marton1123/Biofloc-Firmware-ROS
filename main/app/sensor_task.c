 ...
 * @version 4.1.2 - Thread-Safe hardware layer & NVS wear-leveling protection
 */
...
/* ============================================================================
 * Helper de bloqueo (Mutex)
 * ============================================================================ */
static inline bool lock_sensors(void) {
    if (!s_sensor_mutex) return false;
    return xSemaphoreTake(s_sensor_mutex, pdMS_TO_TICKS(500)) == pdTRUE;
}

static inline void unlock_sensors(void) {
    if (s_sensor_mutex) xSemaphoreGive(s_sensor_mutex);
}
...
esp_err_t sensors_read_ph(sensor_reading_t *reading)
{
    ...
    if (!PH_ENABLED) return ESP_ERR_NOT_SUPPORTED;
    ...
    float ph_value = local_slope * v_sensor + local_offset;
...
esp_err_t sensors_read_temperature(sensor_reading_t *reading)
{
    ...
    if (!TEMP_ENABLED) return ESP_ERR_NOT_SUPPORTED;
    if (!s_ctx.initialized) return ESP_ERR_INVALID_STATE;
    ...
    if (lock_sensors()) {
        if (s_ctx.temp_cal.enabled) {
            local_slope = s_ctx.temp_cal.slope;
            local_offset = s_ctx.temp_cal.offset;
        }
        unlock_sensors();
    }

    float temp_value = local_slope * temp_raw + local_offset;
...
{
    if (!s_ctx.initialized) return ESP_ERR_INVALID_STATE;
    if (slope <= 0.0f) return ESP_ERR_INVALID_ARG;
    ...
    if (lock_sensors()) {
        ...
        ESP_LOGI(TAG, "✓ pH calibration set (manual): pH = %.4f * V + %.4f", slope, offset);
        return ESP_OK;
    }
    return ESP_FAIL;
}
...
{
    if (calib == NULL) return ESP_ERR_INVALID_ARG;
    if (!s_ctx.initialized) return ESP_ERR_INVALID_STATE;
...
static void compute_linear_regression(const calibration_point_t *points,
                                       uint8_t num_points, float *slope,
                                       float *offset, float *r_squared)
{
    if (num_points < 2) {
        *slope = 1.0f; *offset = 0.0f; *r_squared = 0.0f;
        return;
    }
    ...
    if (fabsf(denominator) < 1e-10f) {
        *slope = 0.0f; *offset = sum_y / n; *r_squared = 0.0f;
        return;
    }
    ...
    *offset = (sum_y * sum_xx - sum_x * sum_xy) / denominator;
    ...
    float ss_tot = 0.0f; float ss_res = 0.0f; 
...
{
    if (type >= SENSOR_TYPE_MAX) return ESP_ERR_INVALID_ARG;
    ...
    /* CORRECCIÓN 1: Evitar sobreescritura de NVS si los datos no cambiaron (Wear-Leveling) */
    sensor_calibration_t current_cal_in_nvs;
    size_t required_size = sizeof(sensor_calibration_t);
    bool needs_save = true;
    
    if (lock_sensors()) {
        sensor_calibration_t *cal_in_ram = &s_ctx.calibrations[type];
        
        err = nvs_get_blob(nvs_handle, key, &current_cal_in_nvs, &required_size);
        if (err == ESP_OK) {
            if (memcmp(&current_cal_in_nvs, cal_in_ram, sizeof(sensor_calibration_t)) == 0) {
                ESP_LOGI(TAG, "Calibration identical, skipping NVS write to protect flash.");
                needs_save = false;
            }
        }

        if (needs_save) {
            err = nvs_set_blob(nvs_handle, key, cal_in_ram, sizeof(sensor_calibration_t));
            if (err == ESP_OK) {
...
esp_err_t sensors_load_calibrations(void)
{
    ...
    for (sensor_type_t type = 0; type < SENSOR_TYPE_MAX; type++) {
        ...
        if (err == ESP_OK) {
            ...
            ESP_LOGI(TAG, "✓ Loaded calibration: %s", sensors_get_type_name(type));
            loaded_count++;
        }
    }
...
{
    if (calib == NULL || type >= SENSOR_TYPE_MAX) return ESP_ERR_INVALID_ARG;
    if (!s_ctx.initialized) return ESP_ERR_INVALID_STATE;
...
{
    if (!s_ctx.initialized) {
        if (response) response->status = CAL_STATUS_NOT_INITIALIZED;
        return ESP_ERR_INVALID_STATE;
    }
    if (type >= SENSOR_TYPE_MAX || points == NULL || num_points < 2 || num_points > MAX_CALIBRATION_POINTS) {
        if (response) response->status = CAL_STATUS_INSUFFICIENT_POINTS;
        return ESP_ERR_INVALID_ARG;
    }

    for (uint8_t i = 0; i < num_points; i++) {
        if (points[i].voltage <= 0.0f) {
            if (response) response->status = CAL_STATUS_INVALID_POINTS;
            return ESP_ERR_INVALID_ARG;
        }
    }
    ...
    if (lock_sensors()) {
        ...
        /* CORRECCIÓN 3: Tiempo relativo seguro basado en Uptime del Gateway (Sample_ID) */
        uint32_t uptime_sec = (uint32_t)(esp_timer_get_time() / 1000000ULL);
        snprintf(cal->timestamp, sizeof(cal->timestamp), "uptime_sec:%lu", (unsigned long)uptime_sec);
    ...
    if (response) {
        ...
        snprintf(response->message, sizeof(response->message),
                 "Calibration successful: R²=%.4f, slope=%.6f, offset=%.6f", r_squared, slope, offset);
    }
